...
export const LANGUAGES: Language[] = [
  ...
  { id: "0178", name: "L0178", description: "Learnosity Data API cookbook — recipes for reading from a Learnosity Item bank server-to-server. Use ONLY when the user names Learnosity.", routingHint: "Developer cookbook for the Learnosity Data API, the server-to-server interface to an Item bank. The client describes a data job — which operation, what the request carries, and how far it reads — and L0178 validates it, flags holes as steering warnings, and via get_spec returns a host-language-neutral recipe: goal, preconditions, procedure, a dedicated paging section, gotchas, and verification steps. EARLY: only READING Items from an Item bank (itembank/items, action get) is modelled. Writes, the async job family, and the sessions/reports endpoints are NOT built — do not route those here; say no dialect covers them. Its focus is the truncated read: a short result set returns HTTP 200 with a well-formed body, and meta.records counts the page rather than the total, so a paged job must declare whether it reads one page or to exhaustion. Does NOT author item content (that is L0176), does NOT cover the Author API authoring experience (that is L0177), does NOT call any Learnosity API, and does NOT emit runnable code.", domains: ["learnosity", "integration"], gatedBy: ["learnosity"], status: "Beta" },
];
