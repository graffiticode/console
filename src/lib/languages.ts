...
export const LANGUAGES: Language[] = [
  ...
  // L0179 is L0166 rewritten in the attribute-list style (see docs/language-authoring-style.md)
  // and compiles to byte-identical output — 127 of L0166's 129 corpus programs verified
  // identical by l0179's tools/differential-test.mjs. It is the intended successor, but it is
  // `hidden` rather than routable until that is proven by a sweep: two languages both claiming
  // spreadsheets would make the scope gate mis-route, and L0166 keeps the domain until handover.
  // At handover this entry takes L0166's `domains` and routingHint, L0158/L0176's
  // `composesWith: ["0166"]` repoint here, and L0166 is marked Deprecated.
  { id: "0179", name: "L0179", description: "Spreadsheets (attribute-list successor to L0166)", routingHint: "Interactive spreadsheet authoring with tabular cell data, cell-level formatting, formulas (SUM, AVERAGE, ROUND, IF), parameterized values, and optional assessment validation with per-cell points. Same compiled form as L0166; differs only in source syntax.", domains: [], status: "Beta", hidden: true },
];
