...
export const LANGUAGES: Language[] = ([
  ...
  // Deprecated 2026-09-23 in favour of L0183, a rebuild on L0000 with a new surface (attribute
  // lists, a tray derived from the answer key, scored responses). NOT source-compatible, so
  // L0169 stays registered and keeps serving every item already authored in it; deprecation
  // only stops it being OFFERED (see listLanguages). The steer lives in the TEXT, since
  // `status` is only a label.
  ...
  // answer key. Its output is learnosity-cqt's cell-scoring shape and it serves question.js /
  // scorer.js, so L0176 embeds it as `custom [lang "0183" …]` — hence "0183" in L0176's
  // composesWith. Its keywords are L0169's, so "concept map" requests route here.
  { id: "0183", name: "L0183", description: "Concept webs", routingHint: "Concept webs, concept maps and mind maps laid out as a central hub with related ideas on a circle around it and labelled lines between them — as a study diagram, or as an assessment where the learner drags answers from a tray onto blank nodes and blank line labels, scored with partial credit. Does NOT do flowcharts, timelines, trees or Venn diagrams (Venn is L0171). For a Learnosity item, L0176 embeds an L0183 web.", keywords: ["concept map", "mind map", "graphic organizer"], domains: ["assessments", "diagrams"], status: "Beta" },
] as Language[])
