...
export const LANGUAGES: Language[] = [
  ...
  // L0182 is the first survey language, and the one it will be confused with is L0180: both
  // present a question and collect a response. The line is that a survey response is never right
  // or wrong. Its routingHint therefore spends most of its length disowning assessment rather
  // than describing surveys, and phrases those clauses with the keywords the MCP server's
  // limitSentences() greps for (`do NOT`, `does NOT`, `not built yet`, `never`) — a negative
  // clause without one never reaches the server instructions, and L0180 absorbs the request.
  //
  // `domains: ["surveys"]` introduces that domain; L0182 is currently its only member.
  //
  // `hidden` until the language server is live. Registering it makes it routable, and
  // https://l0182.graffiticode.org does not exist yet — so an unhidden entry would advertise a
  // language whose scope.json 404s and whose create_item fails, and would actively pull
  // survey-shaped requests away from a language that at least answers. Hidden is exactly the
  // case the flag documents: registered everywhere, withheld from the catalog and the
  // planner's candidate list, with findLanguageById still resolving so items render.
  //
  // REMOVE `hidden: true` once l0182.graffiticode.org serves scope.json.
  { id: "0182", name: "L0182", description: "Collective-intelligence surveys", routingHint: "Collective-intelligence surveys, also called group ideation, brainstorming or idea ranking: a participant is shown a sample of ideas drawn from a shared pool, selects the ones they prefer, ranks those selections, may contribute one idea of their own, and sees the group's live ranking. Use it for gathering and prioritizing ideas from a group — what should the team focus on next, which issues matter most, what would improve this — where the participants themselves supply the options and the point is to find out which ones the group favours. Each participant sees a different adaptive sample (typically 10 of many), which is what lets the pool grow without bound, and their micro-rankings are stitched into one ranking predicting how the whole pool would be ordered if everyone had seen all of it. Anonymous and asynchronous by design. An activity is a list of items — a welcome screen, the selection step with its own floor and ceiling on how many may be picked, a drag-to-reorder ranking step, a one-idea contribution that may be optional, a results screen showing scores and a participant count, and a closing message — with QTI's navigation and submission modes over them. The same activity is taken by people in a browser and by AI agents through MCP tools, into the same pool; each participation records which, so the two populations stay separable and a results screen can show the ranking for everyone, for humans only, or for agents only. This is NOT an assessment language and does NOT score anyone: a survey response is never right or wrong, and there is no answer key, no points and no marking. Do NOT route a quiz, test, exam, practice question, comprehension check or any question with a correct answer here — that is L0180. Conventional questionnaires are not built yet: Likert scales, rating and satisfaction questions, demographics, free-text surveys with fixed questions, contact or sign-up forms and branching questionnaire logic are not built, so do not route those here. L0182 declares the survey's parameters and does NOT own the idea pool, draw the sample or compute the ranking — a collective-intelligence service does that, and the activity names the session it belongs to. It never analyses the responses: significance testing, clustering and sentiment analysis are out of scope. Ranking anything other than a line of text is not built yet, and nothing here enforces one response per person.", domains: ["surveys"], status: "Beta", hidden: true },
];
