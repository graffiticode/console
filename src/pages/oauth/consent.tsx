...
// Storage keys for persisting OAuth state across redirect
const STORAGE_KEY_CALLBACK_URL = 'oauth_consent_callback_url';
const STORAGE_KEY_STATE = 'oauth_consent_state';
const STORAGE_KEY_APP_NAME = 'oauth_consent_app_name';
...
export default function OAuthConsent() {
  ...
  // Get values from URL params or sessionStorage (for redirect return)
  const callbackUrl = (Array.isArray(callback_url) ? callback_url[0] : callback_url)
    || (typeof window !== 'undefined' ? sessionStorage.getItem(STORAGE_KEY_CALLBACK_URL) : null);
  const stateParam = (Array.isArray(state) ? state[0] : state)
    || (typeof window !== 'undefined' ? sessionStorage.getItem(STORAGE_KEY_STATE) : null);
  const appName = (Array.isArray(app_name) ? app_name[0] : app_name)
    || (typeof window !== 'undefined' ? sessionStorage.getItem(STORAGE_KEY_APP_NAME) : null)
    || 'an application';
  ...
  useEffect(() => {
    const checkRedirectResult = async () => {
      try {
        ...
        const result = await getRedirectResult(oauthAuth);

        if (result && result.user) {
          setProcessingRedirect(true);

          // Get stored callback URL and state
          const storedCallbackUrl = sessionStorage.getItem(STORAGE_KEY_CALLBACK_URL);
          const storedState = sessionStorage.getItem(STORAGE_KEY_STATE);

          if (!storedCallbackUrl || !storedState) {
          ...
          sessionStorage.removeItem(STORAGE_KEY_CALLBACK_URL);
          sessionStorage.removeItem(STORAGE_KEY_STATE);
          sessionStorage.removeItem(STORAGE_KEY_APP_NAME);
  ...
  }, [router.isReady, loading, callbackUrl, stateParam]);
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      // Store OAuth params in sessionStorage (they'll be lost during redirect)
      sessionStorage.setItem(STORAGE_KEY_CALLBACK_URL, callbackUrl);
      sessionStorage.setItem(STORAGE_KEY_STATE, stateParam);
      sessionStorage.setItem(STORAGE_KEY_APP_NAME, appName);

      const oauthAuth = getOAuthAuth();
      const provider = new GoogleAuthProvider();

      // Use redirect instead of popup
      await signInWithRedirect(oauthAuth, provider);
    } catch (err: any) {
        ...
        <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
          <div className="sm:mx-auto sm:w-full sm:max-w-md">
            ...
            <p className="mt-2 text-center text-sm text-gray-600">
              {processingRedirect ? 'Completing sign-in...' : 'Loading...'}
            </p>
          </div>
