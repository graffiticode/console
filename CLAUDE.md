...
- **Composition** — **reactive, and opted into by a language's own `instructions.md`.** Nothing decides to compose before generation: the head generates atomically, and if the program it wrote emits `data use "<lang>"`, that binding *is* the request to compose. `composesWith` on the `LANGUAGES` entry (`src/lib/languages.ts`) is the hard fence — a **permission, not a trigger** (`composesWithFor`/`fenceComposition`); an unpermitted binding is refused. One live edge: `L0176:["0179"]` (Learnosity item embeds a spreadsheet widget); every other language is atomic. Bindings already present in an item's source are grandfathered at edit time, so withdrawing an edge never strands live content. If an upstream fails, the head is regenerated ONCE standalone with a directive to author inline — the user never sees an upstream's `OUT_OF_SCOPE:` refusal for a language they didn't name. Flag: `COMPOSITION_ENABLED`.

  The pre-flight LLM planner (`planSequence`/`planComposition`/the L0010 planning-RAG corpus) was deleted 2026-09-09. It ran on every fresh create whose head declared an edge and guessed from catalog blurbs before any code existed; a request to L0173 for a bar chart came back as "L0170 is a data-transformation dialect … it has no charting capabilities". L0010 stays registered and keeps compiling — it is simply never called. See `docs/language-routing-and-composition.md` §4 for the permission/trigger split.
